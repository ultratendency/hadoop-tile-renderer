...
public class InverseDistanceWeightingLite {
    ...
    public InverseDistanceWeightingLite(HashMap<DirectPosition2D, Float> positions, Envelope envelope, int xNumOfCells,
            int yNumOfCells) {
        this.env = envelope;
    ...
    private float calculateValue(double[] ds) {
        ...
        for (int i = this.points.length; --i >= 0; ) {
            double dist = Math.sqrt((this.points[i][0] - ds[0]) *  (this.points[i][0] - ds[0])
